...
import { db } from '#/db'
import { conversation, dmAccess, message, user } from '#/db/schema'
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const enriched = await Promise.all(
          conversations.map(async (conv) => {
            const otherId =
              conv.senderId === myId ? conv.receiverId : conv.senderId
            ...
            const accessResult = await db
              .select({ type: dmAccess.type })
              .from(dmAccess)
              .where(
                or(
                  and(eq(dmAccess.senderId, myId), eq(dmAccess.receiverId,otherId)),
                  and(eq(dmAccess.senderId,otherId), eq(dmAccess.receiverId, myId))
                ),
              )
              .orderBy(desc(dmAccess.createdAt))
              .limit(1)
            return { ...conv, other, lastMessage, unreadCount: unread.length ,type: accessResult[0]?.type }
          }),
        )
      ...
      POST: async ({ request }) => {
        ...
        const dmType: 'paywall' | 'guaranteed' =
          type === 'guaranteed' ? 'guaranteed' : 'paywall'
        const paidAmount =
          dmType === 'guaranteed'
