  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const convResult = await db.select().from(conversation).where(eq(conversation.id, id))
        const conv = convResult[0]
        if (!conv) return Response.json({ message: "Not found" }, { status: 404 })
        if (conv.senderId !== myId && conv.receiverId !== myId) return Response.json({ message: "Forbidden" }, { status: 403 })
        
        const messages = await db.select().from(message)
          .where(eq(message.conversationId, conv.id))
          .orderBy(message.createdAt)
          
        await db.update(message).set({ isRead: true }).where(
          and(
      ...
      POST: async ({ request, params }) => {
        ...
        if (!content?.trim()) return Response.json({ message: "Missing content" }, { status: 400 })
        ...
        const msgResult = await db.insert(message).values({
          ...
          content: content.trim(), createdAt: new Date(), isRead: false,
        }).returning()
        const msg = msgResult[0]
        
        let updatedConv = conv
        if (conv.type === "guaranteed" && conv.receiverId === myId && conv.status === "pending") {
          const res = await db.update(conversation)
            .set({ status: "replied", updatedAt: new Date() })
            .where(eq(conversation.id, conv.id)).returning()
          updatedConv = res[0]
        } else {
