...
import { api, type Stats, type UnreadConversation } from "../../lib/api";
import {
  MessageSquare, DollarSign, RefreshCw, ArrowRight,
  Inbox, TrendingUp, Sparkles,
} from "lucide-react";
...
function Avatar({ name, image, size = "md" }: { name: string | null; image: string | null; size?: "sm" | "md" }) {
  ...
  if (image) return <img src={image} referrerPolicy="no-referrer" className={`${sz} rounded-full object-cover flex-shrink-0 ring-2 ring-border`} alt={name || ""} />;
  return (
    <div className={`${sz} rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center font-bold text-primary flex-shrink-0`}>
  ...
  return (
    <Card className={`stat-card animate-fade-up shadow-sm group`}>
      {/* Gradient accent line */}
        <Icon className={`size-8 border group-hover:bg-accent/10 mb-4 transition group-hover:border-accent/20 rounded-lg bg-muted p-2`} />
        <p className="font-lora text-2xl font-md text-foreground tracking-tight">{value}</p>
...
function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([api.stats(), api.unread()])
      .then(([s, u]) => { setStats(s); setUnread(u); })
  ...
  return (
    <div className="w-full pt-14">
      ...
      <div className="mb-8 animate-fade-up">
        ...
        <p className="text-muted-foreground text-sm">Here's what's happening in your inbox today.</p>
      </div>
      ...
      <Card className="overflow-hidden ">
        <div className="flex items-center justify-between pb-4 border-b border-border bg-muted/20">
          <div className="flex items-center gap-2.5">
            ...
            {!loading && unread.length > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-primary text-primary-foreground text-[10px] font-bold min-w-[20px] text-center">
                {unread.length}
              </span>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-6">
            ...
            <p className="text-xs text-muted-foreground">No unread messages right now.</p>
          </div>
        ) : (
          <div className="divide-y divide-border">
            {unread.map((conv, idx) => (
              <a key={conv.id} href={`/messages?conv=${conv.id}`}>
                <div className={`flex items-center gap-3.5 px-5 py-4 hover:bg-muted/40 transition-colors cursor-pointer group animate-fade-up`}
                  style={{ animationDelay: `${idx * 50}ms` }}>
                  <div className="relative  flex-shrink-0">
                    <Avatar name={conv.other?.name ?? null} image={conv.other?.image ?? null} />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 ${conv.type === "guaranteed"
