...
import {
  ...
  Sparkles, Zap, Star, TrendingUp, Heart, Shield, RefreshCw, Users, BadgeCheck
} from "lucide-react";
...
export function BackgroundOrbs() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute -top-40 -right-40 w-96 h-96 rounded-full opacity-[0.07] dark:opacity-[0.12]"
        style={{ background: "radial-gradient(circle, oklch(0.546 0.215 264.1), transparent 70%)" }} />
      <div className="absolute top-1/2 -left-32 w-80 h-80 rounded-full opacity-[0.05] dark:opacity-[0.08]"
        style={{ background: "radial-gradient(circle, oklch(0.65 0.2 295), transparent 70%)" }} />
      <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full opacity-[0.04] dark:opacity-[0.07]"
        style={{ background: "radial-gradient(circle, oklch(0.546 0.215 264.1), transparent 70%)" }} />
    </div>
  );
}
...
/* ─── Animated counter ────────────────────────────────────────────────────── */
export function Counter({ target, prefix = "", suffix = "" }: { target: number; prefix?: string; suffix?: string }) {
...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-background backdrop-blur-3xl`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link href="/">
          <div className="flex items-center gap-2.5 cursor-pointer group">
            <Logo size={28} />
            <span className=" font-medium text-foreground text-base group-hover:text-primary transition-colors">TrueInbox</span>
          </div>
        </Link>

        <div className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          {[["#features", "Features"], ["#faq", "FAQ"]].map(([href, label]) => (
...
export function DashboardMockup() {
  const messages = [
    { name: "NovaBrand", msg: "We'd love to feature you in our Q4...", type: "paywall", amount: "$25", initials: "N", delay: "delay-200" },
    { name: "Alex K.", msg: "Quick question about your workflow?", type: "guaranteed", amount: "$50", initials: "A", delay: "delay-300" },
    { name: "GearUp Co.", msg: "Partnership proposal for next month...", type: "paywall", amount: "$30", initials: "G", delay: "delay-400" },
  ];
      ...
      <div className="relative rounded-[1.75rem] overflow-hidden border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] shadow-[0_30px_80px_rgba(0,0,0,0.38)]">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/10 bg-black/10">
          <div className="flex items-center gap-1.5">
            <Logo size={16} />
            <span className="text-[10px] font-bold text-foreground ">TrueInbox</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-5 h-5 rounded-full bg-primary/15 border border-primary/30 flex items-center justify-center text-[8px] font-bold text-primary">J</div>
        ...
        <div className="p-4 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.01))] space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { label: "Refunded", value: "$120", color: "text-muted-foreground" },
            ].map((s) => (
              <div key={s.label} className="rounded-xl p-3 border border-white/10 bg-black/10 hover:border-primary/20 transition-colors">
          ...
          <div className="rounded-xl border border-white/10 bg-black/10 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
              <span className="text-[9px] font-semibold text-foreground">New Messages</span>
              <span className="text-[8px] text-primary bg-primary/10 px-1.5 py-0.5 rounded-full font-semibold">3 unread</span>
            </div>
            {messages.map((m) => (
              <div key={m.name} className={`flex items-center gap-2.5 px-3 py-2.5 border-b border-white/10 last:border-0 hover:bg-white/[0.03] transition-colors animate-fade-up ${m.delay}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <p className="text-[9px] font-semibold text-foreground">{m.name}</p>
                    <span className={`text-[7px] px-1 py-0.5 rounded font-medium ${m.type === "guaranteed" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                      }`}>
                ...
                <span className="text-[9px] font-mono-nums font-bold text-foreground flex-shrink-0">{m.amount}</span>
              </div>
            ))}
          </div>
        </div>
...
export function FAQ() {
  ...
  return (
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className={`rounded-2xl border transition-all duration-200 overflow-hidden ${open === i ? "border-primary/30 bg-primary/[0.02]" : "border-border bg-card"
          }`}>
          <button
            className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-muted/30 transition-colors"
            onClick={() => setOpen(open === i ? null : i)}
          >
            <span className=" font-semibold text-foreground text-sm pr-6">{item.q}</span>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 ${open === i ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
              }`}>
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${open === i ? "rotate-180" : ""}`} />
            </div>
          </button>
...
export function StatsStrip() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {stats.map((s) => (
        <div key={s.label} className="rounded-3xl border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] px-5 py-6 text-center">
          <p className=" text-3xl font-bold text-foreground">
            <Counter target={s.value} suffix={s.suffix} />
          </p>
          <p className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground mt-2">{s.label}</p>
        </div>
      ))}
    </div>
  );
